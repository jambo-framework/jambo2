...
public class BOFactory {
	...
	public static final Logger log = LogManager.getLogger(BOFactory.class);
	...
	private static AbstractBO useBean(Class controlBeanClass, DBAccessUser user, String transactionAttribute) throws Exception {
		...
		if(PROPAGATION_REQUIRED.equals(transactionAttribute)){
			...
			bo = (AbstractBO) SpringContextManager.getBean(controlBeanClass.getName());
		}else{
			SpringContextManager.registerBean(controlBeanClass.getName() + transactionAttribute, controlBeanClass);
			bo = (AbstractBO) SpringContextManager.getBean(controlBeanClass.getName() + transactionAttribute);
		}
//		bo.setUser(user); // 设置user， 业务bean需要
		return (AbstractBO) bo;
